...
def train_model(model, X, y, j, df, lrate=0.01, epochs=100, report_epoch=10):
    ...
    for i in range(epochs):
        ...
        if i % report_epoch == 0:
            Epoch = f"Epoch {i} Loss"
            Loss = f"{loss}"
            print(f"Epoch {i} Loss = {loss}")
            df = df.append(
                {"Id": j, "Model": modelstr[0:9], "Epoch": Epoch, "Loss": Loss},
                ignore_index=True,
            )
    ...
    if modelstr[0:9] == "AnnLinear":
        df.to_csv(
            "./output/AnnLinear_training_model.csv", mode="a", header=False, index=False
        )
    if modelstr[0:9] == "AnnOneHid":
        df.to_csv(
            "./output/AnnOneHid_training_model.csv", mode="a", header=False, index=False
        )

    if modelstr[0:9] == "AnnTwoHid":
        df.to_csv(
            "./output/AnnTwoHid_training_model.csv", mode="a", header=False, index=False
        )
...
def test_model(model, X, y):
    ...
    examples["Correct"] = [
        1 if targ == pred else 0 for targ, pred in zip(examples["Y"], examples["YHat"])
    ]
